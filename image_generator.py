...
def process_alkkagi_frame_v5(image_src, json_data, resolution):
    ...
    # 1. 이미지 로드 및 리사이즈
    img = cv2.resize(image_src, (resolution, resolution), interpolation=cv2.INTER_AREA)
    ...
    for s in env['stones']['my_team']:
        ...
        for i, deg in enumerate(range(0, 360, 15)):
            ...
            # 색상 지정 (4분면)
            if 0 <= i < 6: color = (0, 0, 255)      # 빨강 계열
            elif 6 <= i < 12: color = (0, 255, 255) # 노랑 계열
            elif 12 <= i < 18: color = (0, 255, 0)  # 초록 계열
            else: color = (255, 255, 0)            # 하늘 계열
            ...
            # [수정 사항] (255, 255, 255) 대신 위에서 정의한 color 변수 사용
            # 가독성을 위해 두께를 thickness에 맞춰 조정 가능
            cv2.putText(result, text, (tx, ty), cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, max(1, thickness//2), cv2.LINE_AA)
